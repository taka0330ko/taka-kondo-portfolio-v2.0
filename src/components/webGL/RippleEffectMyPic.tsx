...
function RippleEffectMyPic() {
  ...
  useEffect(() => {
    ...
    const init = (THREE: typeof import("./three-lite")): (() => void) => {
      ...
      // Texture (image)
      let imageTex: Texture | null = null
      let textureReady = false

      // Reuse the already-loading fallback <img> as the texture source so the
      // photo is only downloaded once; the fallback stays visible until the
      // WebGL render is ready.
      const fallbackImage =
        host.querySelector<HTMLImageElement>("[data-mypic-fallback]")
      const sourceImage =
        fallbackImage ??
        (() => {
      ...
      sourceImage
        ...
        .then(() => {
          ...
          const tex = new THREE.Texture(sourceImage)
    ...
    // fallback <img> keeps the photo visible until the WebGL canvas is ready
  ...
  return (
    <div>
      <div
        ref={threeHostRef}
        className="relative w-120 md:w-130 lg:w-150 aspect-square overflow-hidden rounded-lg
    select-none
